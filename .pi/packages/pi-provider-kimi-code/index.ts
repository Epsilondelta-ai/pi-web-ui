...
// Debug logging
// =============================================================================
function kimiDebug(...args: unknown[]) {
  if (process.env.KIMI_CODE_DEBUG === "1" || process.env.PI_KIMI_CODE_DEBUG === "1") {
    console.error("[kimi-coding]", ...args);
  }
}
...
async function refreshKimiAuthToken(currentKey: string): Promise<string | null> {
  try {
    ...
    if (!cred || cred.type !== "oauth") {
      kimiDebug(`auth refresh skipped: no OAuth credentials for ${PROVIDER_ID} on disk`);
      return null;
    }
    ...
    if (cred.access !== currentKey && Date.now() < cred.expires) {
      kimiDebug("auth refresh: reusing newer on-disk token");
      return cred.access;
    }

    kimiDebug("auth refresh: requesting new access token");
    ...
    kimiDebug("auth refresh: new token persisted");
    return newCred.access;
  } catch (err) {
    kimiDebug("auth refresh failed:", err);
    return null;
  }
}
      ...
      try {
        for await (const event of filterEmptyResponseStream(upstream)) {
          ...
          if (!pushedAny && attempt === 0 && event.type === "error") {
            kimiDebug("upstream error on first event, attempting refresh:", event.error?.errorMessage?.slice(0, 200));
            const refreshed = await refreshKimiAuthToken(currentKey);
            if (refreshed && refreshed !== currentKey) {
              kimiDebug("retrying stream with refreshed token");
              currentKey = refreshed;
              shouldRetry = true;
              break;
            }
            kimiDebug("refresh did not yield a new token, forwarding original error");
          }
          filtered.push(event);
          pushedAny = true;
        }
      } catch (err) {
        kimiDebug("stream error:", err);
        filtered.push({
          type: "error",
          reason: "error",
          error: {
